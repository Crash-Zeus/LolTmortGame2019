...
async def loltmort(ctx):
    gages = ["rename discord une semaine xXx_d4Rk-sASuk3_xXx",
             ...
             "Au prochain challenge 2 4 6 8, si quelqu'un fail il se voit attitrer le titre nul en math dans son pseudo sur discord pendant 24h",
             ...
             "Pokérap ! le mort doit chanter le pokérap pendant un combat",
...
async def echecmode(ctx, player = None):
    if player is None:
        ...
        pion = random.choice(pions)
        await ctx.send(f'{joueur} joueras le ' + str(pion))
